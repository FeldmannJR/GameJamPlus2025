...
namespace Adrenak.UniVoice.Samples
{
    ...
    /// </summary>
    public class UniVoice : MonoBehaviour
    {
        ...
        bool Setup()
        {
            ...
            if (!failed)
                ...
                Debug.unityLogger.Log(LogType.Error, TAG,
                    $"Refer to the notes on top of {GetType().Name}.cs for setup instructions.");
            return !failed;
        }
        ...
        bool SetupAudioServer()
        {
            ...
            AudioServer.OnServerStart += () => { Debug.unityLogger.Log(LogType.Log, TAG, "Server started"); };

            AudioServer.OnServerStop += () => { Debug.unityLogger.Log(LogType.Log, TAG, "Server stopped"); };
            return true;
#else
        ...
        bool SetupClientSession()
        {
            ...
            client.OnJoined += (id, peerIds) => { Debug.unityLogger.Log(LogType.Log, TAG, $"You are Peer ID {id}"); };
            ...
            client.OnPeerJoined += id => { Debug.unityLogger.Log(LogType.Log, TAG, $"Peer {id} joined"); };
            ...
            else
            {
                ...
                _mic = Mic.AvailableDevices[0];
                Debug.unityLogger.Log(LogType.Log, TAG, "Started recording with Mic device named." +
                                                        _mic.Name +
                                                        $" at frequency {_mic.SamplingFrequency} with frame duration {_mic.FrameDurationMS} ms.");
                input = new UniMicInput(_mic);
